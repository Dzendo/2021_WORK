 ...
 * 2. а) там стартовать SkyApplication.kt - что он и делает (значки, иконки и тема bar) (см.ниже)
 *    б) стартовать SkyActivity.kt - находит, загружает, стартует и отдает ей управление
 *    в) применяет стили и названия указанные в манифесте
 *    г) разрешает доступы запрошенные в манифесте
 * 3. SkyActivity.kt - начинает исполняться сверху вниз по строчкам как обычно и при этом:
 *    а) Создает свою ViewModel + repozitory + Dao + DataBase (или связывается с ними)
 *    б) Надувает главный экран из своего activity_sky.xml и размещает на него:
 *      - верхний бар
 ...
 *      - создает и надувает левую шторку заголовком nav_header_sky.xml и
 ...
 *      SkyFragment, зовет его на это место и отдает ему управление.
 *      (SkyActivity отходит в стэк назад и работает оттуда)
 * 5a. SkyFragment, получив управление начинает выполняться сверху вниз с целью разместить себя
 * в выделенном месте. Для этого:
 *    а) Создает свою ViewModel + repozitory + Dao + DataBase (или связывается с ними)
 *    б) Добавляет и обрабатывает меню три точки для этого фрагмента (додувая его sky.xml)
 *       и определяет его обработку
 *    в) Вниз отведенного места надувает нижнее меню bottom_nav_menu.xml и определяет его обработку
 *       в SkyViewModel
 *    г) Еще создает FAB и определяет ее обработку в SkyViewModel
 *    д) Берет свой sky_recycler_list.xml надувает его и размещает его в это место, в середину картины
 *    e) Передает sky_recycler_list.xml ссылку на SkyViewModel, (это технология Buinding), что бы
 ...
 *  ему надо его разместить, а поле recycler view для каждой своей строчки зовет SkyPageAdapter.kt,
 *  Этот адаптер раздувает вид каждой строчки из sky_recycler_item.xml
 *  !!! Если портрет, то строка надувается sky_recycler_item.xml, если ландшафт, то
 *  строка надувается sky_recycler_item.xml (lang)
 *  5в. SkyPageAdapter.kt заполняет строчку/карточку нужными данными, теми, которые сейчас должны
 ...
 *  меню буковку меняет SkyViewModel. Фрагмент мониторит за изменением буковки и когда поменялась
 ...
 *  этой строчки буюсит в SkyViewModel.kt. Фрагмент мверху мониторит вот этот кликнутый номер, который
 *  нормально null:
 *   - как только он не null, то фрагмент вызывает FactDeiailFragment и передает ему этот ID строчки
 *   на которую нажали.
 *   5ж. Если нажали на FAB, то SkyFragment то же зовет FactDeiailFragment, только передает ему
 ...
 *    - буковки PAEMI живут в SkyCONSTANTS.kt enum - это спецкласс
 *    - SkyInjectorUtils - утилиты для создания ViewModel+repo+dao
 ...
 *
 * Этот Application вызывается сразу из манифеста, т.к. android:name=".launcher.SkyApplication",
 * а после него вызывается android:name=".launcher.SkyActivity"
 ...
 */
@HiltAndroidApp
class SkyApplication : Application() { // родительский класс public class Application extends ContextWrapper implements ComponentCallbacks2
    ...
    * Андроид загружает в память класс SkyApplication и создает его экземпляр
    ...
    */
    ...
    private fun timberInit() { // Инициализация Timber
        ...
        /*val cor1 = CoroutineScope(Dispatchers.Default).launch {
            Timber.i("SkyApplication timber Init START_1")
            for (i in 0..10) {
                delay(7000L)
                Timber.i("SkyApplication Работа 1 шаг $i")
            }
            Timber.i("SkyApplication timber Init FINISH_1")
        }

        CoroutineScope(Dispatchers.Default).launch {
            Timber.i("SkyApplication timber Init START_2")
            for (i in 0..10) {
                delay(3000L)
                Timber.i("SkyApplication Работа 2 шаг $i")
            }
            cor1.cancel()
            Timber.i("SkyApplication timber Init FINISH_2")
        }*/
    }
