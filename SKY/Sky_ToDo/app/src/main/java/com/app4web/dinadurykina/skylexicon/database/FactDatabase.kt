...
package com.app4web.dinadurykina.skylexicon.database

import android.content.Context
import androidx.room.Database
import androidx.room.Room
import androidx.room.RoomDatabase
import com.app4web.dinadurykina.skylexicon.launcher.BASE_IN_MEMORY
import com.app4web.dinadurykina.skylexicon.launcher.FACT_TODO_DATABASE_NAME
...
/**
* вызывается из SkyInjectorUtils для создания/подключения базы данных
...
* Fragment <- ViewModel <- ViewModelFactory <- provideSkyActitityViewModelFactory
...
abstract class FactDatabase : RoomDatabase() {
    ...
    companion object {           // Это статика, если говорить по java
        ...
        private fun buildDatabase(context: Context): FactDatabase =
            if (BASE_IN_MEMORY) // Это из SkyCONSTANTS.kt
                // Создать новую базу в памяти
            Room.inMemoryDatabaseBuilder(context, FactDatabase::class.java)
                    .fallbackToDestructiveMigration()
                    .build()
            else
                // Открыть на диске или создать, если ее нет с именем FACT_TODO_DATABASE_NAME из SkyCONSTANTS.kt
 ...
 * В нашем случае первое открытие / создание базы данных идет в SkyActivity при создании ViewModel
 * Фрагменты создавая свои ViewModels тоже вызывают getInstance, но т.к. SkyActivity ее уже открыла,
 * то они получают ссылку на нее.
 * При повороте смартфона SkyActivity и Fragment будет убит, вместе со ссылкой на открытую базу данных,
 * но ссылка убита не будет она хранится тут в :  private var instance: FactDatabase?
 * Создастся новый SkyActivity, он получит ссылку на не убитую ViewModel, а она получит ссылку на открытую базу.
