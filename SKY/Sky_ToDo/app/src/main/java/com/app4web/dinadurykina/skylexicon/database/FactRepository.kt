...
import com.app4web.dinadurykina.skylexicon.launcher.FilterDateEnd
import com.app4web.dinadurykina.skylexicon.launcher.FilterDateStart
...
class FactRepository @Inject constructor (private val factDao: FactDatabaseDao) {
   // private val applicationScope = CoroutineScope(Dispatchers.Default)
    ...
    // Вызывается из SkyActivityViewModel и наблюдается в SkyActivity (это LifeData)
    ...
    // вызывается из SkyViewModel для построения factsPage: Flow<PagingData<Fact>> через поток пейдинг 3.0
 ...
 //==================================заполнение базы ===============================================
 // вызывается из меню ... дозаполнить пачку через SkyViewModel в viewModelScope.launch
     suspend fun addFactDatabase(countFacts: Int = 1000) {
         ...
         withContext(Dispatchers.IO) {
             ...
             for (count in 1..(countFacts / 10_000)) {
                 ...
                 Timber.i("SkyFactRepository Add Database Строк записи =  ${addCount.size} $count --> ${countFacts / 10_000}")
             }
             ...
             Timber.i("SkyFactRepository Add Database Последняя запись = $countFacts * 8 = ${addCount.size}")
         }
     }
    ...
    private fun factContent(count: Int = 100): List<Fact>  {
        ...
        Timber.i("SkyFactRepository Add List Строк записи = $count * 8 = ${count * 8}")
