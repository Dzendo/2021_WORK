...
import com.app4web.dinadurykina.skylexicon.database.FactTable
import com.app4web.dinadurykina.skylexicon.databinding.SkyRecyclerItemBinding
...
class SkyPageAdapterTable (private val skyViewModel: SkyViewModel) : PagingDataAdapter<FactTable, FactViewHolderTable>(diffCallbackTable) {
    ...
    override fun onBindViewHolder(factViewHolderTable: FactViewHolderTable, position: Int) {
        factViewHolderTable.bind(skyViewModel, getItem(position))
    }
 ...
 */
class FactViewHolderTable private constructor(private val binding: SkyRecyclerItemBinding)
    ...
    companion object { // аналог STATIC JAVA при вызове создает экземляр класса FactViewHolder(binding)
        ...
        fun from(parent: ViewGroup): FactViewHolderTable {
            ...
            // Надувает layout\sky_recycler_item.xml - портрет или layout-land\sky_recycler_item.xml - ландшафт
            // В них layout есть окружающий поэтому можно надувать databinding
            val binding = SkyRecyclerItemBinding.inflate(layoutInflater, parent, false)
     ...
     */
    fun bind(skyViewModel: SkyViewModel, item: FactTable?) {
        binding.fact = item                                 // в *_item.xml в переменную факт дает ссылку на факт, который высвечивать
        binding.viewmodel = skyViewModel               // в *_item.xml в переменную viewmodel дает ссылку на skyViewMode, которую вызывать
...
/*class FactListenerTable(val clickListenerTable: (factId: Int) -> Unit) {
    // вызывается из layout\sky_recycler_item.xml через onClick лямбду
