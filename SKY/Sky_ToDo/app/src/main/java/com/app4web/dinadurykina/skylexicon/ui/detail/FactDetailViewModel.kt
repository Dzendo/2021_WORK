...
import com.app4web.dinadurykina.skylexicon.database.Fact
   ...
   // private val viewModelJob = Job()
    ...
    init {
        viewModelScope.launch {
            ...
            Timber.i("Sky FactDetailViewModel $factID")
        }
    ...
    // все три функции обработка выбора пунктов из меню трех точек (вызывается из фрагмента)
     fun update() {
        viewModelScope.launch {
         ...
         Timber.i("Sky Detail ViewModel update ${fact.value?.factId} ")
     ...
     fun insert()  {
        ...
        Timber.i("Sky Detail ViewModel insert ${fact.value?.rezult}")
         backupTrue()
     }
     ...
     fun delete() {
      ...
      //  withContext(Dispatchers.IO) {
         ...
         Timber.i("Sky Detail ViewModel delete ${fact.value?.factId}")
         backupTrue()
     }
