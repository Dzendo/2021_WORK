...
// Стандартный класс ViewModel для фрагмента SkyFragment через Hilt
@HiltViewModel
class SkyViewModel @Inject internal constructor(
    ...
    val adapterPageTable = SkyPageAdapterTable(this)
   ...
   fun onFactClicked(factid: Int) {
    ...
    fun fabClick() {
        ...
        Timber.i("Skytimber SkyFragment Recycler ViewModel fabClick() SnackbarTrue() ${paemi.value?.name}")
    }
    ...
    fun onClickBottomNavView(clickpaemi: MenuItem): Boolean {
        ...
        Timber.i("SkyViewModel onClickBottomNavView ${paemi.value?.name}}")
        return true
    }
