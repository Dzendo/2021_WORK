...
/**
 * Заготовка для будущего использования по умной codelabs:
 * Все подля одной строчки базы нужны будут в форме detail, там мы их и считываем сейчас
 ...
 * из этих 5-7-ми полей и под Recycler Читаем из базы только эти поля от каждой строчки, экономится память, скорость и комильфо
 * Android SQLite @Entity Имеет для этого даже особую форму записи, т.е. это распространенный стандарт
 * так и надо делать, так и буду делать, но потом, когда разберусь с ключами, это предусмотрено, это хорошо, это правильно
 */
