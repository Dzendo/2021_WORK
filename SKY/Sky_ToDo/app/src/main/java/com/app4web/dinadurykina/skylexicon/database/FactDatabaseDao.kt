...
interface FactDatabaseDao {
    ...
    // Вызывается из репо, а оно из ViewModel, а оно наблюдается из SkyActivity
    // Справочно: количество строк в таблице:  SELECT count(*) FROM employee;
    // это обращение к базе данных SQL Lite c просьбой дать количество записей всех в базе fact_sky
    @Query("SELECT COUNT(*) FROM fact_sky")
     ...
     * используется
     */
    @Query("SELECT * from fact_sky WHERE factId = :id")
    ...
    @Query("SELECT * from fact_sky WHERE factId = :id ")  // Limit 1
    ...
    @Query("SELECT factId,data,parent,paemi,nameShort,rezult FROM fact_sky ORDER BY factId DESC")
    ...
    @Query("SELECT factId,data,parent,paemi,nameShort,rezult FROM fact_sky ORDER BY data ASC")
    ...
    @Query("SELECT factId,data,parent,paemi,nameShort,rezult FROM fact_sky WHERE paemi = :paemi" +
