 ...
 * 3. ToDoActivity.kt - начинает исполняться сверху вниз по строчкам как обычно и приэтом:
 *    а) Создает свою ViewModel + repozitory + Dao + DataBase (или связывается с ними)
 *    б) Надувает главный экран и размещает на него:
 *      - верхний бар
 ...
 *      - надувает главный экран из своего activity_to_do.xml
 ...
 *      механизм навигации NavHostFragment и файл навигации MobileNavigate.xml
 * 4. NavHostFragment считывает файл MobileNavigate.xml и определив оттуда, что стартовым будет
 * ToDoFragment зовет его на это место и отдает ему управление. (ToDoActivity отходит в стэк назад
 * и работает оттуда)
 * 5a. ToDoFragment, получив управление начинает выполняться сверху вниз с целью разместить себя
 * в выделенном месте. Для этого:
 *    а) Создает свою ViewModel + repozitory + Dao + DataBase (или связывается с ними)
 *    б) Добавляет и обрабатывает меню три точки для этого фрагмента (надувая его to_do.xml)
