...
object ToDoInjectorUtils {
     ...
     fun getFactRepository(context: Context): FactRepository {
        return FactRepository.getInstance(
                FactDatabase.getInstance(context.applicationContext).factDatabaseDao)
     }

    fun provideToDoActitityViewModelFactory(
       context: Context
    ): ToDoActivityViewModelFactory {
        return ToDoActivityViewModelFactory(getFactRepository(context))
    }

    fun provideToDoViewModelFactory(
        context: Context
    ): ToDoViewModelFactory {
        return ToDoViewModelFactory(getFactRepository(context))
    }

    fun provideFactDetailViewModelFactory(
        context: Context,
        factId: Long
    ): FactDetailViewModelFactory {
        return FactDetailViewModelFactory(getFactRepository(context), factId)
    }
